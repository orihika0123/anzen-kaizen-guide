...
describe("TODOアプリ", () => {
  ...
  it("初期表示", async () => {
    await expect(page).toMatchElement("#todoEmpty", { text: "タスクがありません" })
    await expect(page).toMatchElement("#nextTodo", { text: "次のTODO: (未登録)" })
  ...
  it("タスクの入力", async () => {
    ...
    await expect(page).toMatchElement("#nextTodo", { text: "次のTODO: サンプルタスク1" })
  })
  ...
  it("タスクの削除", async () => {
    ...
    await expect(page).toMatchElement("#nextTodo", { text: "次のTODO: サンプルタスク2" })
    await page.click(".todo:nth-child(1) .delete")
    await page.waitForSelector("#todoList", { visible: false })
    await page.waitForSelector("#todoEmpty", { visible: true })
    await expect(page).toMatchElement("#todoCount", { text: "(全0件)" })
